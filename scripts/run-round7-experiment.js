...
const candidatePath = "benchmarks/round7/llm-candidates.json";
...
async function runArtifact(artifact, windows, responsesByWindow, round3Rows, artifactCache, baselineCache) {
  ...
  return {
    ...
    acceptedSample:
      bestHybrid.source === "llm-window" && bestHybrid.brotliBytes < Math.min(terserBaseline.brotliBytes, deterministic.brotliBytes)
        ? {
            windowId: bestHybrid.windowId,
            candidate: bestHybrid.name,
            deltaBrotliVsTerser: bestHybrid.brotliBytes - terserBaseline.brotliBytes,
            deltaBrotliVsRound3: bestHybrid.brotliBytes - deterministic.brotliBytes,
            before: bestHybrid.sourceWindow,
            after: bestHybrid.rewriteCode,
          }
        : null,
    rejected,
...
function toMarkdown(payload, candidateSet) {
  ...
  const samples = payload.rows.map((row) => row.acceptedSample).filter(Boolean);
  if (samples.length > 0) {
    lines.push("", "## Improved LLM Rewrite Samples", "");
    for (const sample of samples) {
      lines.push(
        `### ${sample.candidate}`,
        "",
        `Brotli delta vs Terser: ${sample.deltaBrotliVsTerser}. Brotli delta vs Round 3: ${sample.deltaBrotliVsRound3}.`,
        "",
        "Before:",
        "",
        fenced(sample.before),
        "",
        "After:",
        "",
        fenced(sample.after),
...
async function main() {
  ...
  await writeFile(join(resultsDir, "round7.json"), `${JSON.stringify(payload, null, 2)}\n`);
  await writeFile(join(resultsDir, "round7.md"), toMarkdown(payload, candidateSet));
  ...
  console.log(`\nWrote ${join(resultsDir, "round7.json")} and ${join(resultsDir, "round7.md")}`);
}
